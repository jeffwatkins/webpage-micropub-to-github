...
module.exports = function (githubTarget, micropubDocument, siteUrl, options) {
  ...
  return Promise.resolve(
    ...
    .then(formatter => {
      if (micropubDocument.file) return micropubDocument;
      return formatter.formatAll(micropubDocument);
    })
    .then(formatted => {
      if (!formatted.file) return formatted;
      return publisher.publish(formatted.file.filename, formatted.file.buffer, {
        force: force,
    ...
    .then(formatted => {
      console.log('Fully formatted', formatted);
